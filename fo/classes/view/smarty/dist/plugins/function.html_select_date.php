...
function smarty_function_html_select_date($params, &$smarty)
{
    ...
    $year_empty      = null;
    $extra_attrs     = '';
	$language        = $smarty instanceof CustomSmarty ? reset(explode('_', $smarty->locale)) : 'en';
    ...
    if ($display_months) {
        ...
        for ($i = 1; $i <= 12; $i++) {
			$_loc_from = array();
			$_loc_to   = array();
			if (isset($GLOBALS['strftimeNames'][$language])) {
				if (strpos($month_format, '%B') !== false) {
					$_loc_from[] = '%B';
					$_loc_to[]	 = $GLOBALS['strftimeNames'][$language]['fullMonthNames'][$i];
				}
				if (strpos($month_format, '%b') !== false) {
					$_loc_from[] = '%b';
					$_loc_to[]	 = $GLOBALS['strftimeNames'][$language]['shortMonthNames'][$i];
				}
			}

            $month_names[$i] = strftime(str_replace($_loc_from, $_loc_to, $month_format), mktime(0, 0, 0, $i, 1, 2000));
