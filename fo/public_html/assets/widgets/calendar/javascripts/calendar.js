...
var Calendar = new Class({
	...
	options: {
		...
		classes: ['', '', 'calnext'], // ['calendar', 'prev', 'next', 'month', 'year', 'today', 'invalid', 'valid', 'inactive', 'active', 'hover', 'hilite']
		days: ['Zondag', 'Maandag', 'Dinsdag', 'Woensdag', 'Donderdag', 'Vrijdag', 'Zaterdag'], // days of the week starting at sunday
	...
	initialize: function(obj, options) {
		...
		for (var i = 0; i < obj.length; i++) {
			...
			if (!this.element(cal.el, obj[i], cal)) { continue; }
	...
	element: function(el, f, cal) {
		if ($defined(f.inputs)) { // in the case of multiple inputs per calendar
			if ($type(f.inputs) == 'object')
				f.inputs = [f.inputs];
			for (var i = 0; i < f.inputs.length; i++) {
				if (!this.element(f.inputs[i].elem, f.inputs[i], cal)) { return false; }
			}
	...
	write: function(cal) {
		...
		cal.els.each(function(el) { el.fireEvent('change'); });
	}
});
