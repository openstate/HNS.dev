var Form = new Class({
	initialize: function(form, checks, events, giveFocus) {
		...
		if ($pick(giveFocus, true)) {
			var inputs = this.form.getElements('input[type!=hidden], textarea, select');
	...
	disable: function(elem, enabled) {
		elem.getElements('input, textarea, select').each(function(el) {
			el.disabled = !enabled;
		});
	},
...
var FocusHints = new Class({
	initialize: function(elem) {
		...
		this.hints.each(function(el) {
			...
			var inputs = el.getPrevious().getElements('input, textarea, select').addEvent('focus', function(ev) {
				el.setStyle('display', '');
