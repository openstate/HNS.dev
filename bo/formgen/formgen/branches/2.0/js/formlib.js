var Form = new Class({
	initialize: function(form, checks, events, giveFocus) {
		...
		for (var i = 0; i < events.length; i++) {
			events[i].binder(events[i].event, events[i].handler.bindAsEventListener(this, [this.form]));
		...
		this.form.getElements('.tab-block').each(function(el) {
			this.tabs.push(new SimpleTabs(el, { entrySelector: 'h4' } ));
		}, this);
	},
	...
	setVisibility: function(elem, visible) {
		...
		if (elem.hasClass('error')) {
			this.tabs.each(function(tabGroup) {
				tabGroup.entries.each(function(tab) {
