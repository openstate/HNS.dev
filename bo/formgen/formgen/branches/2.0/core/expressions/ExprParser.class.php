	...
	public function nextToken() {
		...
		if ($ch=='\'') { // String
			...
			do {
				...
				$this->pos++;
				$continue = $this->pos + 1 < strlen($this->expr) && substr($this->expr, $this->pos, 2) == '->';
				if ($continue) {
					$this->pos++;
					$str.= '->';
					if ($this->pos + 1 >= strlen($this->expr))
						throw new ParseException('Incomplete identifier at position '.($this->pos+1).': '.$str);
				}
			} while ($continue);
...
class ExprParser {
	...
	*/
	public function __construct($expr, $idCallback) {
	...
	*/
	protected function parseFactor() {
		...
		if ($this->accept(ExprTokenizer::Operator, '(')) {
			...
			return call_user_func($this->idCallback[0], $value, $this->idCallback[1]);
		} else {
			if ($this->token->token == ExprTokenizer::EOT)
				$val = 'EOT';
			else
