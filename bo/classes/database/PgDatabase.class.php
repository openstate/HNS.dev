...
require_once 'database/Database.abstract.php';
...
class PgDatabaseResult extends DatabaseResult {
	...
	public function __destruct() {
		pg_free_result($this->queryHandle);
	}

	public function fetchCell() {
		return @pg_fetch_result($this->queryHandle, 0, 0);
	}

	public function fetchRow() {
		return @pg_fetch_assoc($this->queryHandle);
	}

	protected function initFieldNames() {
		if ($this->fieldTables) return;

		for ($i = 0; $i <pg_num_fields($this->queryHandle); $i++) {
	...
	public function fetchRowByTable() {
		$this->initFieldNames();
		$row = @pg_fetch_row($this->queryHandle);
		if (!$row)
			return false;
		$result = array();
	...
	protected function fetchAssoc() {
		return pg_fetch_all($this->queryHandle);
	}

	protected function fetchFirstColumn() {
		return pg_fetch_all_columns($this->queryHandle, 0);
	}

	public function affectedRows() {
		return pg_affected_rows($this->queryHandle);
	}

	public function numRows() {
		return pg_num_rows($this->queryHandle);
	}
}
