...
abstract class Database {
	...
	*/
	...
	/*
		Constant: IDENTIFIER
		Specifies that a variable should be escaped as a identifier
	*/
	...
	*/
	protected function escape($value, $type) {
		switch ($type) {
			...
			case self::IDENTIFIER:	return $this->formatIdentifier($value);
			default:
				throw new DatabaseException('Can\'t escape value of type: "'.$type.'"');
		}		
	}

	/*
		Method: formatIdentifier
		Formats an identifier (table/column/alias name)

		Parameters:
		$identifier - The identfier to format

		Return:
		The formatted identifier

	*/
	abstract public function formatIdentifier($identifier);
	
	/*
		Method: concat
		concatenates the parameters
		note: It uses the parameters as is.

		Parameters:
		* - The values to concatenate

		Return:
		The database concatenated values
	...
	*/
	...
	/*
		...
		t - format the parameter as table/identifier		
		% - Inserts a literal %.
	...
	*/
	protected function formatSql($sql, $params) {
		...
		while (($pos=strpos($sql, '%', $prev)) !== false) {
			...
			$escaped = true;
			if ($prev < strlen($sql)) {
				if ($param === null && strpos('ifbsxtl', $sql[$prev]) !== false)
					...
					case 't': $param = $this->escape((string)$param,  self::IDENTIFIER);$prev++; break;					
					case 'l':                                                        	$prev++; break;
	...
	*/
	...
	/*
		...
		Placeholders in the query are allowed, either all ? or named placeholders following the :name format
