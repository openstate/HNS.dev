...
class MyDatabaseResult extends DatabaseResult {
	public function __destruct() {
		if (is_resource($this->queryHandle)) {
			mysql_free_result($this->queryHandle);
		}
	}

	public function fetchCell() {
		$row = @mysql_fetch_row($this->queryHandle);
		if ($row)
			return reset($row);
		else
			return false;
	...
	public function fetchRow() {
		return @mysql_fetch_assoc($this->queryHandle);
	}

	protected function fetchAssoc() {
		$result = array();
		while ($row = mysql_fetch_assoc($this->queryHandle))
			$result[]= $row;
		return $result;
	}
